    ...
    <div className="bg-white rounded-xl shadow-md p-6">
      ...
      <div className="flex flex-col md:flex-row">
        <div className="space-y-4 flex-1 px-0 md:px-4">
          <InfoRow
            label="หมายเลขใบงาน"
            value={job?.JobId ? `#${job.JobId}` : "-"}
          />
          <InfoRow label="ชื่องาน" value={job?.title || "-"} />
          <InfoRow label="สถานะ" value={job?.status || "-"} isBadge />
          <InfoRow
            label="วันที่สร้าง"
        ...
        <div className="space-y-4 flex-1 px-0 md:px-4 mt-4 md:mt-0">
          ...
          <InfoRow
            label="วันที่สิ้นสุด"
            value={
              job?.end_available_at ? DateFormatWork(job.end_available_at) : "-"
            }
          />
          <InfoRow
            label="เสร็จสิ้น"
          ...
          />
          <InfoRow label="แผนก" value={job?.department?.name || "-"} />
        </div>
      </div>
    </div>
  );
